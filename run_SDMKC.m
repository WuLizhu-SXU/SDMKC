...
for i1 = 1:length(datasetCandi)
    ...
    if ~exist(fname2, 'file')
        ...
        lambdas = 10.^(-5:1:5);
        paramCell = SDMKC_build_param(lambdas);
        nParam = length(paramCell);
        SDMKC_result = zeros(nParam, 1, nRepeat, 13);  
        SDMKC_time = zeros(nParam, 1);
        for iParam = 1:nParam
            disp(['SDMKC_iParam= ', num2str(iParam), ', totalParam= ', num2str(nParam)]);
            fname3 = fullfile(prefix_mdcs, [data_name, '_12kAllFea_SDMKC_', num2str(iParam), '.mat']);
            if exist(fname3, 'file')
                ...
                for iRepeat = 1:nRepeat
                    ...
                    [Y_EXP, obj_value] = SDMKC(Ks_cell, nCluster, param.lambda, NITER);
                   ...
                   SDMKC_result(iParam, 1, iRepeat, :) = [result_10'];
                end
                tt = toc(t1);
               SDMKC_time(iParam) = tt / nRepeat;
                save(fname3, 'SDMKC_result', 'tt', 'param');
            end
        end

        a1 = sum(SDMKC_result, 2);
        a2 = sum(a1, 3);
        a3 = reshape(a2, nParam, 13);
        a4 = a3 / nRepeat;
        SDMKC_result_summary = [max(a4, [], 1), sum(SDMKC_time) / nParam];
        save(fname2, 'SDMKC_result', 'SDMKC_time', 'SDMKC_result_summary');
